...
// HitMarvelsEndpoint provides a mock function with given fields: method, path, params
func (_m *CharacterUsecase) HitMarvelsEndpoint(method string, path string, params string) (models.MarvelsResult, error) {
	ret := _m.Called(method, path, params)

	var r0 models.MarvelsResult
	if rf, ok := ret.Get(0).(func(string, string, string) models.MarvelsResult); ok {
		r0 = rf(method, path, params)
	} else {
		r0 = ret.Get(0).(models.MarvelsResult)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(string, string, string) error); ok {
		r1 = rf(method, path, params)
	} else {
		r1 = ret.Error(1)
	}
